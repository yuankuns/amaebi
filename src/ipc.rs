...
mod tests {
    ...
    // ---- Legacy payloads (removed variants) deserialize as Unknown -----------

    #[test]
    fn legacy_distill_with_branch_field_still_parses() {
        let json = r#"{"type":"distill_claude_prompt","brief":"fix","cwd":"/tmp","model":"sonnet","branch":"feat-x"}"#;
        let r: Request = serde_json::from_str(json).unwrap();
        assert!(matches!(r, Request::DistillClaudePrompt { .. }));
    }
    ...
    #[test]
    fn legacy_worktree_created_deserializes_as_unknown() {
        let json = r#"{"type":"worktree_created","path":"/tmp/wt","branch":"fix-foo-abc123"}"#;
        let r: Response = serde_json::from_str(json).unwrap();
