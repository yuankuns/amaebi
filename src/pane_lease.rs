...
/// path without shelling out.  NOT gated behind `#[cfg(test)]` because
/// integration tests spawn the daemon as a separate binary process
/// where `cfg(test)` is false.  The env var is never set in production.
