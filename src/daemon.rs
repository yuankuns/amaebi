...
/// distillation so the LLM investigates the correct starting point.
/// Responds with `WorktreeCreated` + `Done` or `Error` + `Done`.
...
mod tests {
    ...
    fn distillation_system_prompt_mentions_emit_tool_and_cwd() {
        let p = build_distillation_system_prompt("/tmp/repo", Some("feat-xyz"));
        // Working directory must be interpolated, not left as a placeholder.
        assert!(p.contains("/tmp/repo"), "cwd must be in prompt: {p}");
        // Branch name must appear in the prompt.
        assert!(p.contains("feat-xyz"), "branch must be in prompt: {p}");
