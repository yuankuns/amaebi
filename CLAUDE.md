...
- The rule depends on whether HEAD is on master or on a feature branch; see `scripts/next-version.sh` header for the full rationale.
  ...
  - **Merge commits** are handled transparently: branch mode skips them via `git log --no-merges`; master mode on a merge commit uses `HEAD^1` (first-parent) as the delta anchor.  GitHub's synthetic `refs/pull/<N>/merge` is non-ancestor of master, so it falls into branch mode and is handled by the `master_ref..HEAD` scan.
